...
namespace GameServer.DAO
{
    class UserDAO
    {
        public User VerifyUser(MySqlConnection conn, string username, string password)
        {
            ...
            try
            {
                MySqlCommand cmd = new MySqlCommand("select * from user where account = @username and password = @password", conn);
        ...
        public bool GetUserByUsername(MySqlConnection conn, string username)
        {
            ...
            try
            {
                MySqlCommand cmd = new MySqlCommand("select * from user where account = @username", conn);
                cmd.Parameters.AddWithValue("username", username);
                reader = cmd.ExecuteReader();
                if (reader.HasRows)
                {
        ...
        public void AddUser(MySqlConnection conn, string username, string password)
        {
            try
            {
                MySqlCommand cmd = new MySqlCommand("insert into user set account = @username , password = @password", conn);
                cmd.Parameters.AddWithValue("username", username);
                cmd.Parameters.AddWithValue("password", password);
                cmd.ExecuteNonQuery();
            }
